...
fs.readFile(audioFilePath, (err, data) => {
    ...
    .on('complete', function() {
        ...
        fs.writeFile('pyin.json', json, 'utf8', function (err) {
            if (err) {
                console.log("An error occured while writing PYIN Extractor JSON Object to File.");
                return console.log(err);
            }

            console.log("PYIN Extractor JSON file has been saved.");
        });
    })
    // run async
    .run({ 'async': true });       
