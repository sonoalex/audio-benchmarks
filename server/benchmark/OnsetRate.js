...
fs.readFile(audioFilePath, (err, data) => {
    ...
    suite.add('Essentia#ONSET', () => {
        essentia.OnsetRate(essentia.arrayToVector(audioBuffer));
    }, options)
    // add listeners
    ...
    .on('complete', function() {
        ...
        fs.writeFile('onset.json', json, 'utf8', function (err) {
            if (err) {
                console.log("An error occured while writing Onset Rate JSON Object to File.");
                return console.log(err);
            }

            console.log("Onset Rate JSON file has been saved.");
        });
    })
    // run async
    .run({ 'async': true });       
