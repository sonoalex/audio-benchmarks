...
fs.readFile(audioFilePath, (err, data) => {
    ...
    .on('complete', function() {
        ...
        fs.writeFile('distribution_shape.json', json, 'utf8', function (err) {
            if (err) {
                console.log("An error occured while writing Distribution Shape JSON Object to File.");
                return console.log(err);
            }

            console.log("Distribution Shape JSON file has been saved.");
        });
    })
    // run async
    .run({ 'async': true });       
