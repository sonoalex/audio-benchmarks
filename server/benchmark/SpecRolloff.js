...
fs.readFile(audioFilePath, (err, data) => {
    ...
    const suite = new Benchmark.Suite('SpecRolloff');

    // add tests
    suite.add('Meyda#SpecRolloff', () => {
    ...
    .on('complete', function() {
        ...
        fs.writeFile('spectral_rolloff.json', json, 'utf8', function (err) {
            if (err) {
                console.log("An error occured while writing Spectral Rolloff JSON Object to File.");
                return console.log(err);
            }

            console.log("Spectral Rolloff JSON file has been saved.");
        });
    })
    // run async
    .run({ 'async': true });       
