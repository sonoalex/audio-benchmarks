...
fs.readFile(audioFilePath, (err, data) => {
    ...
    .on('complete', function() {
        ...
        fs.writeFile('yin.json', json, 'utf8', function (err) {
            if (err) {
                console.log("An error occured while writing YIN Extractor JSON Object to File.");
                return console.log(err);
            }

            console.log("YIN Extractor JSON file has been saved.");
        });
    })
    // run async
    .run({ 'async': true });       
