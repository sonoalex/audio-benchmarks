...
fs.readFile(audioFilePath, (err, data) => {
    ...
    const suite = new Benchmark.Suite('SpectralFlatness');

    // add tests
    suite.add('Meyda#SpectralFlatness', () => {
    ...
    .on('complete', function() {
        ...
        fs.writeFile('spectral_flatness.json', json, 'utf8', function (err) {
            if (err) {
                console.log("An error occured while writing Spectral Flatness JSON Object to File.");
                return console.log(err);
            }

            console.log("Spectral Flatness JSON file has been saved.");
        });
    })
    // run async
    .run({ 'async': true });       
